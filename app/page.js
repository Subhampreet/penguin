...
const faqs = [
  {
    ...
    answer:
      "Vital is a powerful project management tool designed to help teams organize, track, and manage their work efficiently. It combines intuitive design with robust features to streamline your workflow and boost productivity.",
  },
  {
            ...
            {features.map((feature, index) => (
              <Card key={index} className="bg-gray-700 border-none">
                <CardContent className="pt-6">
                  <feature.icon className="h-12 w-12 mb-4 text-gray-500" />
          ...
          <Accordion type="single" collapsible className="w-full">
            {faqs.map((faq, index) => (
              <AccordionItem key={index} value={`item-${index}`}>
                <AccordionTrigger className="text-slate-300">{faq.question}</AccordionTrigger>
